...
from mission_control.utils.add_guardrails import dot_matrix_two_dimensional_drone, meters_per_pixel
...
def run(args) -> int:
    ...
    print(f"[image] {photo_path} -> cropped to square {side}x{side} (matches mission_control's crop_img_square)")

    if args.prepare:
        out_path = photo_path.with_name(f"{photo_path.stem}.square.png")
        img.save(out_path)
        print(f"[prepare] Wrote {out_path}")
        print("Open it in an image viewer that shows pixel coordinates on hover, "
              "find two points a known real-world distance apart, then re-run with "
              "--height, --ref-pixel1, --ref-pixel2, --ref-distance.")
        return 0

    if args.height is None or args.ref_pixel1 is None or args.ref_pixel2 is None or args.ref_distance is None:
        print("[error] --height, --ref-pixel1, --ref-pixel2 and --ref-distance are all required "
    ...
    error_pct = (measured_mpp - predicted_mpp) / predicted_mpp * 100 if predicted_mpp else float("inf")
    ...
    print(f"Measured scale (from your {args.ref_distance}m reference over {pixel_dist:.1f}px): "
          f"{measured_mpp:.4f} m/px")
    print(f"Error: {error_pct:+.1f}%")
    print(f"Suggested camera_fov_degrees to match reality: {suggested_fov:.1f} "
    ...
    overlay = dot_matrix_two_dimensional_drone(img.copy(), camera_fov_degrees=args.fov, drone_height=args.height)
    out_path = args.out or str(photo_path.with_name(f"{photo_path.stem}.grid_check.png"))
...
def parse_args(argv=None):
    ...
    p.add_argument("--photo", required=True, help="Path to a photo of a physical ground reference")
    p.add_argument("--prepare", action="store_true",
                   help="Just crop & save the square image for pixel-picking, then exit")
    p.add_argument("--height", type=float, help="Drone/camera height above ground when the photo was taken (m)")
    p.add_argument("--fov", type=float, default=10.8,
    ...
    p.add_argument("--ref-pixel1", help="Pixel coords 'x,y' of first reference point in the cropped square image")
    p.add_argument("--ref-pixel2", help="Pixel coords 'x,y' of second reference point")
    p.add_argument("--ref-distance", type=float, help="Known real-world ground distance between the two points (m)")
    p.add_argument("--tolerance-pct", type=float, default=5.0, help="Max allowed scale error, percent")
    p.add_argument("--out", help="Path to write the annotated overlay image (default: <photo>.grid_check.png)")
