...
class FlySearchVLMBridge(VLMBridge):
    ...
    def __init__(self, config: Config, event_bus: EventBus, storage: ChatStorageHelper):
        ...
        self.event_bus.subscribe(
            CreateNewSessionCommand, self.handle_create_new_session
        )
        ...
        try:
            if chat_id not in self.conversations:
                raise VLMConnectionError(
                    f"Chat with id {chat_id} is not initialized. Use CHAT_INIT first."
                )
            ...
            err_event = VlmErrorOccurred(
                chat_id=chat_id,
                error_message=f"[VLM] Analysis failed: {e!s}",
    ...
    async def handle_create_new_session(self, event: CreateNewSessionCommand):
        ...
        try:
            if chat_id in self.conversations:
                raise VLMConnectionError(
                    f"Chat with id {chat_id} already exists. "
            ...
            err_event = ChatErrorOccurred(
                chat_id=chat_id,
                error_message=f"[VLM] Chat creation failed: {e!s}",
                traceback=traceback.format_exc(),
            )
            await self.event_bus.publish(err_event)
    ...
    async def handle_save_session(self, event: SaveSessionCommand):
        ...
        try:
            ...
            err_event = ChatErrorOccurred(
                chat_id=chat_id,
                error_message=f"[VLM] Saving chat failed: {e!s}",
                traceback=traceback.format_exc(),
            )
            await self.event_bus.publish(err_event)
    ...
    async def handle_load_session(self, event: LoadSessionCommand):
        ...
        try:
            ...
            async with self.chat_locks[chat_id]:
                ...
                self.conversations[chat_id] = await self._list_to_conversation(
                    conversation_list
                )
            ...
            err_event = ChatErrorOccurred(
                chat_id=chat_id,
                error_message=f"[VLM] Loading chat failed: {e!s}",
                traceback=traceback.format_exc(),
            )
            await self.event_bus.publish(err_event)
    ...
    async def _list_to_conversation(
    ...
    async def _execute_transaction(
        ...
        try:
            ...
            raise VLMConnectionError("Message sending to the VLM failed.") from e
