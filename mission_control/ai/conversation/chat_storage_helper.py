...
from mission_control.core.exceptions import ChatRestoreError, ChatSaveError
...
class FileChatStorageHelper(ChatStorageHelper):
    ...
    async def save_chat(self, chat_id: str, conversation: Conversation) -> None:
        ...
        try:
            ...
            raise ChatSaveError(
                f"Failed to create chat directories at {chat_dir}: {e}"
            ) from e
            ...
            if isinstance(content_data, str):
                ...
                try:
                    ...
                    current_message["parts"].append(
                        {"type": "image", "path": relative_path}
                    )
                    image_counter += 1
                except Exception as e:
                    raise ChatSaveError(
                        f"Error processing image for message {role_val}: {e}"
                    ) from e
    ...
    async def load_chat(self, chat_id: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            raise ChatRestoreError(
                f"Failed to read or parse chat history from {json_path}: {e}"
            ) from e
        ...
        try:
                    ...
                    if part["type"] == "text":
                        ...
                        if image_path.exists():
                            ...
                            img_obj = await asyncio.to_thread(
                                load_image_sync, image_path
                            )
                            loaded_parts.append({"type": "image", "data": img_obj})
                        else:
                            logger.warning(
                                f"[CHAT STORAGE] Image not found at {image_path}, skipping."
                            )
            ...
            raise ChatRestoreError(
                f"Failed to open image file during restore: {e}"
            ) from e
