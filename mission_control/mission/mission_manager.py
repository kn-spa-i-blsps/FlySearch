...
from mission_control.core.events import SearchEnded, StartMissionCommand
...
class MissionManager:
    ...
    async def handle_start_mission(self, event: StartMissionCommand):
        ...
        if mission_id in self.active_missions:
            logger.warning(
                f"[MISSION MANAGER] Mission {mission_id} is already running!"
            )
            return
        ...
        try:
            ...
            logger.error(
                f"[MISSION MANAGER] Failed to start mission {mission_id}: {e}",
                exc_info=True,
            )
    ...
    async def handle_mission_ended(self, event: SearchEnded):
        ...
        if mission_id not in self.active_missions:
            logger.warning(
                f"[MISSION MANAGER] SearchEnded event, but mission {mission_id} is not running!"
            )
            return

        logger.info(
            f"[MISSION MANAGER] Deleting Orchestrator for mission: {mission_id}"
        )
