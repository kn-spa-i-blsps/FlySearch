...
from mission_control.core.exceptions import (
    DroneCommunicationError,
    DroneInvalidDataError,
)
...
class WebSocketDroneBridge:
    ...
    def __init__(
        ...
        storage: DataStorageHelper,
    ):
        self.connected_clients: Dict[
            str, Any
        ] = {}  # Active drone connections mapping {drone_id: ws_instance}
        self.disconnected_clients = (
            set()
        )  # Tracks drones that lost connection abnormally
        ...
        self.event_bus.subscribe(
            GetPhotoAndTelemetryCommand, self.handle_get_photo_telemetry
        )
    ...
    async def handler(self, ws):
        ...
        try:
            ...
            logger.warning(
                f"[WS] Error or timeout during authorization with {peer}: {e}"
            )
            return
        ...
        try:
            ...
            async for message in ws:
                try:
                    ...
                    logger.warning(
                        f"[WS] Ignored message (not JSON nor binary): {message}"
                    )
                    continue
            ...
            logger.info(
                f"[WS] Drone disconnected gracefully: {peer}. {self.video_helper.format_disconnect_reason(e)}"
            )
            await self.event_bus.publish(DroneDisconnected(drone_id=drone_id))
        except ConnectionClosedError as e:
            logger.warning(
                f"[WS] Drone connection broken: {peer}. {self.video_helper.format_disconnect_reason(e)}"
            )
            if drone_id:
            ...
            logger.warning(
                f"[WS] disconnected: {peer}. {self.video_helper.format_disconnect_reason(e)}"
            )
        except Exception as e:
            logger.error(f"[WS] error: {e}")
            logger.debug(traceback.format_exc())
        finally:
            self.video_helper.clear_waiters(
                self.video_helper.recording_ack_waiters, "Connection lost before ACK"
            )
            self.video_helper.clear_waiters(
                self.video_helper.recordings_ack_waiters, "Connection lost before ACK"
            )
            ...
            if drone_id is not None and self.connected_clients.get(drone_id) == ws:
                ...
                logger.info(
                    f"[WS] Drone {drone_id} removed from the connected clients registry."
                )
        ...
        try:
            ...
            if event.drone_id in self.disconnected_clients:
                await self.event_bus.publish(
                    DroneConnectionLost(drone_id=event.drone_id)
                )
        except Exception as e:
            error_event = DroneErrorOccurred(
                drone_id=event.drone_id,
                error_message=f"[WS] Failed to send get_photo_telemetry request to drone {event.drone_id}: {e!s}",
                traceback=traceback.format_exc(),
            )
            await self.event_bus.publish(error_event)
            logger.error(
                f"[WS] Failed to send get_photo_telemetry request to drone {event.drone_id}: {e}"
            )
        ...
        try:
            ...
            if move is not None:
                try:
                    ...
                    raise ValueError(
                        f"[WS] Invalid move coordinates {move}: {e}"
                    ) from e
                payload["move"] = [x, y, z]
            else:
            ...
            error_event = DroneErrorOccurred(
                drone_id=drone_id,
                error_message=f"[WS] Failed to send move command to drone {drone_id}: {e!s}",
                traceback=traceback.format_exc(),
            )
            await self.event_bus.publish(error_event)
    ...
    async def handle_start_recording(self, event: StartRecordingCommand):
        ...
        try:
            ...
            await self.event_bus.publish(
                DroneErrorOccurred(
                    drone_id=drone_id,
                    error_message=f"Start recording failed: {e}",
                    traceback=traceback.format_exc(),
                )
            )
    ...
    async def handle_stop_recording(self, event: StopRecordingCommand):
        ...
        try:
            ...
            await self.event_bus.publish(
                DroneErrorOccurred(
                    drone_id=drone_id,
                    error_message=f"Stop recording failed: {e}",
                    traceback=traceback.format_exc(),
                )
            )
    ...
    async def handle_get_recordings(self, event: GetRecordingsListCommand):
        ...
        try:
            ...
            await self.event_bus.publish(
                RecordingsListReceived(
                    drone_id=drone_id, recordings=ack.get("recordings", [])
                )
            )
        except Exception as e:
            await self.event_bus.publish(
                RecordingsListReceived(drone_id=drone_id, error=str(e))
            )
            await self.event_bus.publish(
                DroneErrorOccurred(
                    drone_id=drone_id,
                    error_message=f"Failed to fetch recordings list: {e}",
                    traceback=traceback.format_exc(),
                )
            )
    ...
    async def handle_pull_recordings(self, event: PullRecordingsCommand):
        ...
        try:
            ...
            logger.info(
                f"[WS] Successfully pulled recordings from {drone_id}. Results: {ack.get('processed_results')}"
            )
            await self.event_bus.publish(
                RecordingsPullCompleted(
                    drone_id=drone_id, results=ack.get("processed_results", [])
                )
            )
        except Exception as e:
            await self.event_bus.publish(
                RecordingsPullCompleted(drone_id=drone_id, error=str(e))
            )
            await self.event_bus.publish(
                DroneErrorOccurred(
                    drone_id=drone_id,
                    error_message=f"Pull recordings transfer failed: {e}",
                    traceback=traceback.format_exc(),
                )
            )
    ...
    async def _authenticate_drone(self, ws, peer) -> str | None:
        ...
        if isinstance(first_msg, (bytes, bytearray)):
            logger.warning(
                f"[WS] Expected JSON AUTH payload, but received binary data from {peer}."
            )
        ...
        if auth_obj.get("type") == "AUTH" and "drone_id" in auth_obj:
            ...
            if self.connected_clients.get(drone_id):
                logger.warning(
                    f"[WS] Drone {drone_id} is already connected. Rejecting connection from {peer}."
                )
                await self._send_ack(ws, of="AUTH", ok=False)
                return None
    ...
    @staticmethod
    async def _send_ack(
        ws, of: str, ok: bool = True, seq: int = None, error: str = None
    ) -> None:
        payload = {"type": "ACK", "of": of, "ok": ok}
        if seq is not None:
            payload["seq"] = seq
    ...
    async def _process_message(self, ws, drone_id: str, obj: dict) -> None:
        ...
        match obj:
            ...
            case {
                ...
                logger.info(
                    f"[WS] Received PHOTO_WITH_TELEMETRY from {drone_id} (seq: {seq})"
                )
                try:
                    (
                        photo_path,
                        telemetry_path,
                    ) = await self.storage.save_photo_and_telemetry(photo, telemetry)
                    await self.event_bus.publish(
                        PhotoWithTelemetryReceived(
                            drone_id=drone_id,
                    ...
                    await self._send_ack(
                        ws, of="PHOTO_WITH_TELEMETRY", ok=True, seq=seq
                    )
                except DroneInvalidDataError as e:
                    logger.error(
                        f"[WS] Error saving photo/telemetry for {drone_id}: {e}"
                    )
                    await self._send_ack(
                        ws, of="PHOTO_WITH_TELEMETRY", ok=False, seq=seq, error=str(e)
                    )
            ...
            case _:
                logger.warning(
                    f"[WS] Unrecognized message type from {drone_id}: {obj.get('type')}"
                )
