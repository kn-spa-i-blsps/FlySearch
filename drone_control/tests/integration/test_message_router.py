...
class TestGetPhotoTelemetry:
    def test_sends_immediate_ack_then_payload(self, tmp_path):
        ...
        router.on_message(
            ws,
            json.dumps({"type": "COMMAND", "action": "GET_PHOTO_TELEMETRY", "seq": 5}),
        )
    ...
    def test_photo_none_when_sensor_fails(self, tmp_path):
        router, _, _ = _make_router(
            tmp_path, photo_sensor=_make_photo_sensor(fail=True)
        )
        ws = _ws()

        router.on_message(
            ws,
            json.dumps({"type": "COMMAND", "action": "GET_PHOTO_TELEMETRY", "seq": 1}),
        )
...
class TestMoveCommand:
    ...
    def test_move_executed_sent_even_when_fc_raises(self, tmp_path):
        ...
        router.on_message(
            ws,
            json.dumps(
                {"type": "COMMAND", "action": "MOVE", "move": [0, 1, 0], "seq": 3}
            ),
        )
    ...
    def test_move_calls_flight_controller(self, tmp_path):
        ...
        router.on_message(
            ws,
            json.dumps(
                {"type": "COMMAND", "action": "MOVE", "move": [2.0, 3.0, 4.0], "seq": 1}
            ),
        )
...
class TestRecordingCommands:
    def test_start_recording_ack(self, tmp_path):
        ...
        router.on_message(
            ws, json.dumps({"type": "COMMAND", "action": "START_RECORDING"})
        )
    ...
    def test_stop_recording_ack(self, tmp_path):
        ...
        router.on_message(
            ws, json.dumps({"type": "COMMAND", "action": "STOP_RECORDING"})
        )
    ...
    def test_get_recordings_ack(self, tmp_path):
        ...
        router.on_message(
            ws, json.dumps({"type": "COMMAND", "action": "GET_RECORDINGS"})
        )
...
class TestServerAckHandling:
    def test_server_ack_does_not_trigger_response(self, tmp_path):
        ...
        router.on_message(
            ws,
            json.dumps(
                {"type": "ACK", "of": "PHOTO_WITH_TELEMETRY", "ok": True, "seq": 5}
            ),
        )
