...
def _recording_error_ack(action: str):
    def builder(exc: Exception, seq: int | None) -> dict:
        return {
            "type": "ACK",
            "of": "RECORDING",
            "action": action,
...
def build_registry(
    *,
    ...
    telemetry_sensor: TelemetrySensor | None = None,
    recording_sensor: RecordingSensor | None = None,
) -> CommandRegistry:
...
def pull_recordings(
    ws: Any, obj: dict[str, Any], recording_sensor: RecordingSensor
) -> None:
    names_raw = obj.get("names", [])
    names: list[str] = (
        [n for n in names_raw if isinstance(n, str)]
    ...
    for idx in range(0, len(prepared), batch_size):
        for entry in prepared[idx : idx + batch_size]:
            ...
            try:
                ...
                results.append(
                    {"name": name, "ok": False, "error": f"begin_send_failed: {exc}"}
                )
                continue
